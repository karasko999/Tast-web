...
function renderPlans() {
    ...
    if (filtered.length === 0) {
        // 🔥 إخفاء زر إضافة الخطة للمستخدم العادي
        ...
        const adminButton = isAdmin ? `
            <button class="btn-primary" style="width:auto;padding:12px 35px;margin-top:15px;" onclick="navigateTo('admin')">
                <i class="fa-solid fa-plus"></i> إضافة خطة (أدمن)
            </button>
        ` : '';
        ...
        container.innerHTML = `
            <div class="empty-state-modern">
                ...
                <p class="muted">${isAdmin ? 'قم بإضافة خطط جديدة من لوحة تحكم الأدمن' : 'سيتم إضافة خطط قريباً'}</p>
                ${adminButton}
            </div>
        `;
        return;
