// main.js - التطبيق الرئيسي (بدون إعادة تحميل الصفحة)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            // ✅ إعادة تمكين الزر فوراً (حتى لو نجح)
            btn.disabled = false;
            btn.innerHTML = 'تسجيل الدخول';
            
            showToast(result.message, result.success ? 'success' : 'error');
            
            // ❌ لا نعيد تحميل الصفحة، نعتمد على حدث authChanged
            // if (result.success) {
...
function showApp(user, isAdmin) {
    ...
    // عرض اسم المستخدم (حتى لو كان user فارغاً جزئياً)
...
console.log('✅ main.js (بدون إعادة تحميل) جاهز');